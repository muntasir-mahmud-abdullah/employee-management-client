    ...
    <div className="bg-gray-100 dark:bg-gray-600 py-10">
      ...
      <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        {achievements.map((achievement, index) => (
          <div key={index} className="p-6 bg-white dark:bg-gray-300 rounded shadow text-center">
            <h3 className="text-4xl font-bold text-blue-500">{achievement.title}</h3>
            <p className="mt-2 text-gray-600 dark:text-white">{achievement.description}</p>
          </div>
        ))}
      </div>
    </div>
